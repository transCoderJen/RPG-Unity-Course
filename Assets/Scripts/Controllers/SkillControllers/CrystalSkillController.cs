...
public class CrystalSkillController : MonoBehaviour
{
    ...
    public void SetupCrystal(float _crystalDuration, bool _canExplode, bool _canMoveToEnemy, float _moveSpeed, float _growSpeed, float _maxSize, bool _moveTowardsClosestEnemy, Player _player)
    {
    ...
    private void AnimationExplodeEvent()
    {
        ...
        foreach(var hit in colliders)
        {
            if (hit.GetComponent<Enemy>() != null)
            {
                player.stats.DoMagicDamage(hit.GetComponent<CharacterStats>(), true);

                Inventory.instance.GetEquipment(EquipmentType.Amulet)?.Effect(hit.GetComponent<Enemy>().transform);
